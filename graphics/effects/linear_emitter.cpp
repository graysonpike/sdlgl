...
LinearParticleEmitter::LinearParticleEmitter(Scene *scene, int x, int y,
        ...
        ParticleEmitter(scene, x, y, colors, quantity, size, particle_duration, lifetime, emit_delay),
        vx_min(vx_min),
        vx_max(vx_max),
        vy_min(vy_min),
        vy_max(vy_max) {}
...
std::pair<int, int> LinearParticleEmitter::get_random_velocity() { 
    ...
    float normalizing_factor = sqrt((vx * vx) + (vy * vy)) / (abs(vx) + abs(vy));
...
void LinearParticleEmitter::new_particle() {
    ...
    particles.push_back(new LinearParticle(scene, x, y, particle_size, v.first, v.second, next_color(), particle_duration));
...
void LinearParticleEmitter::set_velocity(int vx_min, int vx_max, int vy_min, int vy_max) {
