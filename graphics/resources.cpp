...
Resources::Resources(SDL_Renderer *renderer) { this->renderer = renderer; }
...
Sound Resources::get_sound(std::string name) { return Sound(sounds[name]); }

Track Resources::get_track(std::string name) { return Track(tracks[name]); }
...
Resources::~Resources() {
    ...
    for (texture_it = textures.begin(); texture_it != textures.end();
         texture_it++) {
    ...
    for (sprite_it = sprite_frames.begin(); sprite_it != sprite_frames.end();
         sprite_it++) {
