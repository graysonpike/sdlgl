...
class ParticleExample {
    ...
    void game_loop(Context context, Scene *scene) {
        if (context.inputs->is_key_down_event(SDL_SCANCODE_SPACE)) {
            scene->add_entity(new LinearParticleEmitter(
   ...
   public:
    int start() {
        Context context(new Graphics(640, 480), new Audio(), new Inputs(),
                        new Clock());
        Scene *scene =
            new Scene(context.graphics, context.audio, context.inputs);
        ...
        LinearParticleEmitter *fire = new LinearParticleEmitter(
