...
Sound::Sound(const std::string& filename, int instances)
    : instances(instances) {
    Audio& audio = Audio::get_instance();
    std::string filepath = sounds_dir + filename;
    for (int i = 0; i < instances; i++) {
        sounds.emplace_back(
            std::shared_ptr<ma_sound>(new ma_sound, ma_sound_uninit));
        ma_result result =
            ma_sound_init_from_file(audio.get_engine(), filepath.c_str(), 0,
                                    NULL, NULL, sounds[i].get());
        if (result != MA_SUCCESS) {
            ...
            std::string message =
                "Failed to initialize sound: " + filepath + reason;
...
void Sound::play() {
    if (sounds.empty()) {
        std::cout << "Error: Attempting to play sound with no data."
                  << std::endl;
    }
    for (int i = 0; i < instances; i++) {
        if (!is_playing(i)) {
    ...
    std::cout << "Reached end of sound instances vector after new play() was "
                 "requested."
              << std::endl;
}
...
void Sound::set_looping(bool looping) {
    if (sounds.size() > 1) {
        std::cout << "Error: Looping functionality should only be used with "
                     "instances = 1."
                  << std::endl;
    }
...
float Sound::get_pitch() { return ma_sound_get_pitch(sounds[0].get()); }
