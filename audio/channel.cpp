...
Channel::Channel(int channel)
    : volume(1.0f), fade_mode(NO_FADE), channel(channel), reserved(false) {}
...
void Channel::update(float delta) {
    switch (fade_mode) {
        case NO_FADE:
            break;
        case TIMED: {
            set_volume(fade_target_volume +
                       (fade_volume_difference * timer.remaining()));
            if (timer.is_done()) {
                fade_mode = NO_FADE;
...
float Channel::get_volume() { return volume; }
...
void Channel::cancel_fade() { fade_mode = NO_FADE; }

bool Channel::is_playing() { return Mix_Playing(channel); }

bool Channel::is_free_and_not_playing() { return !reserved && !is_playing(); }

void Channel::free() { reserved = false; }
