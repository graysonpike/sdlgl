...
TypewriterText::TypewriterText() 
    : full_text(""), current_text(""), font(""), color({0, 0, 0, 255}), 
      x(0), y(0), max_width(-1), delay(0.1f), timer(0.0f), 
      current_char_index(0), is_typing(false), typing_complete(false), char_width(0.0f) {
    calculate_character_width();
    perform_text_wrapping();
}

TypewriterText::TypewriterText(int x, int y, const std::string& text, const std::string& font,
                               float delay, int max_width)
    : full_text(text), current_text(""), font(font), color({0, 0, 0, 255}),
      x(x), y(y), max_width(max_width), delay(delay), timer(0.0f),
      current_char_index(0), is_typing(false), typing_complete(false), char_width(0.0f) {
    calculate_character_width();
    perform_text_wrapping();
}
...
void TypewriterText::render() {
    ...
    // Always pass max_width = -1 to disable SDL's wrapping since we handle it manually
...
bool TypewriterText::is_complete() const {
    return typing_complete;
}
...
void TypewriterText::calculate_character_width() {
    ...
    int texture_width, texture_height;
    SDL_QueryTexture(test_texture.get(), nullptr, nullptr, &texture_width, &texture_height);
    
    // Calculate average character width
    char_width = static_cast<float>(texture_width) / static_cast<float>(test_string.length());
}
...
void TypewriterText::perform_text_wrapping() {
    ...
    for (const std::string& word : words) {
        ...
        if (!current_line.empty() && current_line.length() + 1 + word.length() > chars_per_line) {
